...
try:
    ...
    from ._pyautogui_pyscreeze_opt import center, pixel, pixelMatchesColor, screenshot
...
def _mouseMoveDrag(moveOrDrag, x, y, xOffset, yOffset, duration, tween=linear, button=None):
    ...
    if duration > MINIMUM_DURATION:
        if moveOrDrag == "move" and tween == linear and hasattr(platformModule, "_moveToSmooth"):
            ...
            platformModule._moveToSmooth(x, y, duration, num_steps)
            return
...
def failSafeCheck():
    ...
    if sys.platform == "win32":
        try:
            ...
            triggered = _rust_core.check_failsafe_triggered()
            if triggered:
                _rust_core.reset_failsafe_triggered()
        except Exception:
...
def set_use_driver(use_driver):
    if sys.platform == "win32":
        platformModule.set_use_driver(use_driver)

...
try:
    _rust_core.set_process_dpi_aware()
    _rust_core.start_failsafe_hook()
except Exception:
...
for c in range(32, 128):
    keyboardMapping[chr(c)] = _rust_core.vk_key_scan_a(c)
...
def _keyDown(key):
    ...
    for apply_mod, vk_mod in [(mods & 4, 0x12), (mods & 2, 0x11),
        ...
        if apply_mod:
            events.append((1, vk_mod, 0, KEYEVENTF_KEYDOWN))
    events.append((1, vkCode, 0, KEYEVENTF_KEYDOWN))
    for apply_mod, vk_mod in [(mods & 1 or needsShift, 0x10), (mods & 2, 0x11),
        (mods & 4, 0x12)]: #HANKAKU not supported! mods & 8
        if apply_mod:
            events.append((1, vk_mod, 0, KEYEVENTF_KEYUP))
            
    try:
        _rust_core.send_inputs(events)
    except Exception:
        pass
...
def _keyUp(key):
    ...
    for apply_mod, vk_mod in [(mods & 4, 0x12), (mods & 2, 0x11),
        ...
        if apply_mod:
            events.append((1, vk_mod, 0, KEYEVENTF_KEYDOWN))
    events.append((1, vkCode, 0, KEYEVENTF_KEYUP))
    for apply_mod, vk_mod in [(mods & 1 or needsShift, 0x10), (mods & 2, 0x11),
        (mods & 4, 0x12)]: #HANKAKU not supported! mods & 8
        if apply_mod:
            events.append((1, vk_mod, 0, KEYEVENTF_KEYUP))
...
def _position():
    ...
    """
    try:
        return _rust_core.get_cursor_pos()
    except Exception:
        return (0, 0)
...
def _size():
    ...
    """
    return (_rust_core.get_system_metrics(0), _rust_core.get_system_metrics(1))
...
def _moveToSmooth(x, y, duration, steps):
    try:
        _rust_core.move_to_smooth(x, y, duration, steps)
    except Exception:
        pass
...
def _mouse_is_swapped():
    ...
    return _rust_core.mouse_is_swapped()
...
def _sendMouseEvent(ev, x, y, dwData=0):
    ...
    try:
        _rust_core.send_mouse_event(ev, x, y, dwData)
    except Exception:
        pass
...
def _moveRel(x, y):
    try:
        _rust_core.move_rel(x, y)
    except Exception:
        pass


def set_use_driver(use_driver):
    _rust_core.set_use_driver(use_driver)
